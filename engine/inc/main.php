...
if ( !count($stats_arr) ) {
	...
	$row = $db->super_query( "SELECT COUNT(*) as count FROM " . PREFIX . "_category WHERE active ='1'" );
	$bolimlar['bolimlar'] = number_format( $row['count'], 0, ',', ' ');
...
$row = $db->super_query( "SELECT COUNT(*) as count FROM " . PREFIX . "_claims" );
  ...
  {$lang['anti_box']}
		...
		<div class="panel panel-default">
			...
			<ul class="nav nav-tabs nav-tabs-solid">
				<li class="active"><a href="#statall" data-toggle="tab"><i class="fa fa-bar-chart position-left"></i> Saytning umumiy statistikasi</a></li>
				<li><a href="#notinfo" data-toggle="tab"><i class="fa fa-pencil-square-o position-left"></i>  Shaxsiy eslatmalar</a></li>
	        ...
	        <div class="panel-tab-content tab-content">
	            <div class="tab-pane active" id="statall">
					...
					<div class="panel-footer">
						<a class="btn bg-danger-600 btn-sm btn-raised" href="engine/ajax/controller.php?mod=adminfunction&action=clearcache&user_hash={$dle_login_hash}">Keshni tozalash</a>
					</div>
					</div>
                     <div class="tab-pane has-padding" id="notinfo">
						<textarea id="notice" name="notice" class="classic" style="width:100%;height:200px;" placeholder="Bu yerda siz o'zingizning eslatmalaringizni saqlashingiz mumkin.">{$row['notice']}</textarea>
						<button id="send_notice" name="send_notice" class="btn bg-teal btn-sm btn-raised"><i class="fa fa-floppy-o"></i> Saqlash</button>
                     </div>
...
<div class="panel panel-default">
	<div class="panel-heading">
		 Saytning umumiy statistikasi
	</div>
	<div class="panel-tab-content tab-content">
		<div class="tab-pane active" id="statall">
			<table class="table table-sm">
			<tr>
				<td class="col-md-3">Sayt holati</td>
				<td class="col-md-9">Barqaror ishlamoqda</td>
			</tr>
			<tr>
				<td>Saytdagi ma'lumotlar soni</td>
				<td>{$stats_arr['stats_news']} ta</td>
			</tr>
			<tr>
				<td>Murojaatlar soni</td>
				<td>{$c_complaint} [ <a href="{$config['http_home_url']}admin.php?mod=claims" target="_blank">Murojaatlarni ko'rish</a> ]</td>
			</tr>
			<tr>
				<td>Bo'limlar soni</td>
				<td>{$bolimlar['bolimlar']} [ <a href="{$config['http_home_url']}admin.php?mod=categories" target="_blank">Bo'limlarni  ko'rish</a> ]</td>
			</tr>
			</table>
		</div>
	</div>
