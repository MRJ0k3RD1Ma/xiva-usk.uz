...
HTML;
		...
		$entries .= "<td style=\"text-align: center\" class=\"cursor-pointer\" onclick=\"document.location = '?mod=editnews&action=editnews&id={$row['id']}'; return false;\">";
...
<div class="panel panel-default">
  <div class="panel-heading">
    ...
    <table class="table table-striped table-xs table-hover">
      <thead>
      <tr>
        <th class="hidden-xs hidden-sm" style="width: 60px;">Sana</th>
        <th>{$lang['edit_title']}</th>
        <th class="hidden-xs text-center" style="width: 60px;">Ko'rilgan</th>
        <!-- <th class="hidden-xs text-center" style="width: 60px;"><i class="fa fa-comment-o tip" data-original-title="{$lang['edit_com']}"></i></th> -->
        <th style="width: 30px;text-align:center;">Holati</th>
        <th class="hidden-xs text-center">Bo'limi</th>
        <th class="hidden-xs hidden-sm" style="max-width: 140px">Muallif</th>
	...
	<div class="panel-footer">
			  <div class="pull-right">
				<select name="action" class="uniform position-left">
					...
					<option value="mass_date">Bugungi sanani joriy etish</option>
					<option value="mass_delete">O'chirish</option>

				</select><input class="btn bg-teal btn-sm btn-raised" type="submit" value="Amalni bajarish">
			  </div>
	</div>
</div>
HTML;
...
HTML;
    ...
    function confirmDelete(url, id){
		...
		$(\"body\").append(\"<div id='dlepopup' title='Tasdiqlash' class='dle-promt' style='display:none'><div id='dlepopupmessage'>Joriy ma'lumotni o'chirishni xohlaysizmi?</div></div>\");
	...
	if( $user_group[$member_id['user_group']]['allow_main'] ) $main_input = "<div class=\"checkbox\" id=\"opt_holder_main\"><label><input class=\"icheck\" type=\"checkbox\" id=\"allow_main\" name=\"allow_main\" value=\"1\" {$ifmain}>Bosh sahifaga chiqarish</label></div>"; else $main_input = "";
                 ...
                 <div class="panel-tab-content tab-content">
                     <div class="tab-pane active" id="tabhome">
						<div class="panel-body">
							...
							<div class="form-group">
							  <label class="control-label col-sm-2">Ma'lumot</label>
							  <div class="col-sm-10">
								<span class="position-left">ID=<b>{$row['id']}</b>, Ma'lumot muallifi</span>{$author_info}
							  </div>
							 </div>

							<div class="form-group">
							  <label class="control-label col-sm-2">Sarlavha</label>
							  <div class="col-sm-10">
								<input type="text" class="form-control width-550 position-left" name="title" id="title" value="{$row['title']}" maxlength="250">
							  </div>
							 </div>

							<div class="form-group">
							  <label class="control-label col-sm-2">Sana</label>
							  <div class="col-sm-10">
								<input type="text" name="newdate" data-rel="calendar" class="form-control position-left" style="width:190px;" value="{$row['date']}" autocomplete="off"><label class="checkbox-inline"><input class="icheck" type="checkbox" name="allow_now" id="allow_now" value="yes">Bugungi sanani belgilash</label>
							  </div>
							 </div>

							<div class="form-group">
							  <label class="control-label col-sm-2">Bo'lim</label>
							  <div class="col-sm-10">
							 ...
							 <div class="form-group editor-group">
							  <label class="control-label col-md-2">Ma'lumot matni</label>
							  <div class="col-md-10">
HTML;
...
// End XFields Call
	...
	if( $config['allow_admin_wysiwyg'] ) {
		...
		echo "<div class=\"editor-panel\"><div class=\"shadow-depth1\">{$bb_code}<textarea class=\"editor\" style=\"width:100%;height:600px;\" onfocus=\"setFieldName(this.name)\" name=\"short_story\" id=\"short_story\" >{$row['short_story']}</textarea></div></div>";
	}
...
{$output}
							...
							<div class="form-group">
							  <label class="control-label col-md-2">Tahrirlash sababi</label>
							  <div class="col-md-10">
								<div class="checkbox"><label><input class="icheck" type="checkbox" id="view_edit" name="view_edit" value="1" {$view_edit_cheked}>Ma'lumot tahrirlanganligi haqidagi ma'lumotlarni ko'rsatish</label></div><input type="text" class="form-control width-450 position-left" name="editreason" id="editreason" value="{$row['reason']}">{$lang['news_edit_date']}
							  </div>
							 </div>
				...
				<div class="panel-footer">
					...
					{$spam_btn}
					<button onclick="confirmDelete('?mod=editnews&action=doeditnews&ifdelete=yes&id=$id&user_hash=$dle_login_hash', '{$id}'); return false;" class="btn bg-danger btn-sm btn-raised"><i class="fa fa-trash-o position-left"></i>O'chirish</button>
